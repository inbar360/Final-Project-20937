...
from utils import create_aes_key, create_uuid, create_directory, get_client_file_path
...
def handle_one_param(server: Server, client_id: bytes, code: int, unpacked_payload) -> ReqState:
    """
    ...
    """
    name: str = decodes_utf8(unpacked_payload)
...
def handle_sending_public_key(server: Server, client_id: bytes, code: int, unpacked_payload: tuple) -> ReqState:
    """
    ...
    :return: The response code generated by the server.
    """
    name, public_key = unpacked_payload
    name = decodes_utf8(name)
...
def handle_sending_file(server: Server, client_id: bytes, code: int, unpacked_payload: tuple) -> ReqState:
    """
    ...
    :return: The response code generated by the server.
    """
    content_size, orig_size, pack_num, tot_packets, file_name, content = unpacked_payload
    client: Client = server.get_client(client_id)
    file_name = decodes_utf8(file_name)
...
def register(server: Server, name: str) -> ReqState:
    """
...
def reconnect(server: Server, client_id: bytes, name: str) -> ReqState:
    """
...
def crc_requests(server: Server, client_id: bytes, code: int, file_name: str) -> ReqState:
    """
